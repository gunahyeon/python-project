...
class SearchView(ListView):
    ...
    # 검색어 결과 가져오기
    def get_queryset(self):
        query = self.request.GET.get('query', '') # 주소창에 빈문자열 검색처리는 : '' 사용
...
class CommentCreateView(LoginAndVerificationRequiredMixin, CreateView):
    http_method_names = ['post'] # post 요청만 받겠다
...
class ProcessLikeView(LoginAndVerificationRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # like 를 눌렀는지 확인하는 법 : url로 넘어온 값 가져오기
        # get_or_create : 좋아요 눌러져있으면 created false, 좋아요가 없으면 created true
        ...
        # 제네릭좋아요 누르고 리다이렉트 할 때, 완전히 다른페이지에서도 request 할 수 있기 때문에 , request가 날라온 페이지로 리다이렉트
